	...
	<body>
		...
		<div id="background">
			<?php			
				...
				$userObject->checkIfUserHasGemeindeRight(6);
				...
				<p class="mainTextDoidSans">
					<?php
						...
						if($_SESSION)
						{
							...
							$sucess 	= $gemeinde->updateInformationsleiste($inhalt);
							
							if($sucess)
							{
								$gemeinde_id = $gemeinde->GemID."('".$gemeinde->Name."');";
								$GLOBALS["system"]->logger->outputLog("[ADMIN_GEMEINDE] Informationsleiste von Gemeinde '$gemeinde_id' wurde bearbeitet - ".$userObject->getFullName());
