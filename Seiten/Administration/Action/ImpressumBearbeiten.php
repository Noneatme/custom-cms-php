			...
			<?php include include $_SERVER['DOCUMENT_ROOT']."/Home/Design/Main_Start.php"; ?> 
				...
				<p class="mainTextDoidSans">
						<?php
							...
							$GLOBALS["system"]->logger->outputLog("[ADMIN_HAUPTSEITE] Impressum wurde bearbeitet - ".$userObject->getFullName());
			
							
							if(isset($_POST))
							{
								$text = $GLOBALS["system"]->getDatabaseClass()->escapeString($_POST['impressum']);
								$GLOBALS["system"]->getDatabaseClass()->query("UPDATE tblsystem SET Impressum = '$text';");
							}
